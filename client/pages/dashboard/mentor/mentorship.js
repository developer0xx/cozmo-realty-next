...
const mentorship = ({ mentees, userInfo }) => {
	const classes = useStyles();
	const [rows, setRows] = useState(mentees);
	...
	const handleRoleFilter = async role => {
		...
		setRows(await fetchMentees(role));
		setSelected([]);
	};
...
const fetchMentees = async (uid, cookieHeader) => {
	return await axios({
		method: 'GET',
		url: `${API}/mentor/get/mentees/${uid}`,
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
			cookie: cookieHeader,
		...
		.then(res => {
			console.log(`=====================mentorship.js 389=================`, res.data.mentees);
			if (res.data.mentees) {
				return res.data.mentees.map((mentee, i) => ({
					id: i + 1,
		...
		.catch(console.log('Failed to fetch mentees'));
};
...
export const getServerSideProps = async ctx => {
	...
	const mentees = await fetchMentees(userInfo._id, cookieHeader);
	console.log(`==================mentorship.js 410 ==============`, mentees);
	return { props: { userInfo, mentees } };
};
