...
const UserAccountUpdate = props => {
	const currUser = props.userAccountInfo;
	...
	const roles = ['admin', 'agent', 'agent-pending', 'coldcaller', 'coldcaller-pending', 'customer', 'mentor', 'mentor-pending', 'membership', 'new-comer'];

	const [userRole, setUserRole] = useState(Object.assign({}, ...roles.map(role => ({ [role]: currUser.role.includes(role) ? true : false }))));
	...
	const onSubmitHandler = e => {
		...
		let newRoles = [];
		Object.keys(userRole).forEach((key, i) => {
			if (userRole[key]) {
				newRoles.push(key);
			}
		});

		body = { ...body, role: newRoles };
	...
	if (!User.email) {
		...
		return (
			<Container fluid id="admin-user-update-form">
				<Row className="form-paper">
					<Col xs={12} sm={8} lg={6} xl={5}>
						...
						<form className="form-wrapper" onSubmit={onSubmitHandler}>
							...
							<Col className="form-input p0">
								<h3 className="form-title">Role</h3>
							</Col>
							<Col className="form-input p0">
								{Object.keys(userRole).map((key, i) => {
									return (
										<FormControlLabel
											key={i}
											control={<Checkbox onChange={handleRole} value={userRole[key]} name={key} checked={userRole[key]} color="primary" />}
											label={key}
										/>
									);
								})}
							...
							<Col className="form-input p0">
								<h4 className="form-title">Account Management</h4>
							</Col>
							<Col className="form-input p0">
								<TextField
									variant="outlined"
