	...
	const deleteHandler = async (event, selected) => {
		...
		await axios({
			...
			if (response.data.success) {
				...
				console.log('Failed to delete leads');
			}
		});
	};
...
const fetchLeads = async uid => {
	return await axios.get(`${API}/leads/coldcaller/get/all/${uid}`, { withCredentials: true }).then(res => {
		...
		<DashboardLayout userInfo={userInfo}>
			<div className={classes.root}>
				<Paper className={classes.paper}>
					<UsersToolbar numSelected={selected.length} selected={selected} rows={rows} />
					<TableContainer>
...
export const getServerSideProps = async ctx => {
	...
	const rows = await fetchLeads(userInfo._id);
